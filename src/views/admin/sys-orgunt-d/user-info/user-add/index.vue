...
export default {
  ...
  methods: {
    ...
    submitForm(foconstame) {
      ...
      ApiObj.orgusers(params).then(res => {
        if (res.code === 0) {
          this.$message({
            type: 'success',
    ...
    handleSelectionChange(val) {
      const orgInfo = JSON.parse(window.sessionStorage.getItem('org-user-info'))
      this.selectedList = val
      this.selectedListArr = this.selectedList.map(item => {
        return { ...item, orgCodg: orgInfo.orgCodg, orguntId: orgInfo.orguntId, orgName: orgInfo.orgName }
      })
    },
    // 切换每页的数量
