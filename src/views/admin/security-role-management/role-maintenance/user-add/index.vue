...
export default {
  ...
  methods: {
    ...
    getUserList() {
      ...
      // let id = this.dataObj.roleId
      this.userTableLoading = true
      ApiObj.securityRoleUsers(params).then(res => {
        if (res.code === 0) {
    ...
    submitForm(foconstame) {
      ...
      ApiObj.securityRoleUsersSave(params).then(res => {
        if (res.code === 0) {
          this.$message({
            type: 'success',
    ...
    handleSelectionChange(val) {
      ...
      this.selectedListArr = this.selectedList.map(item => {
        return { ...item, roleId: this.securityRoleId, roleType: '1', roleName: this.dataObj.searchForm.roleName }
      })
    },
    // 切换每页的数量
