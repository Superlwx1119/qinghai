...
<template>
  <!-- 组织机构变更 -->
  <normal-layer :search-number="itemsDatas.length" title-name="组织机构变更列表" title-need-handle>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      columnsLabel: ['组织机构名称', '组织机构编码', '医保区划', '简称', '变更类型', '经办人', '经办时间'],
      columnsValue: ['orgName', 'orgCodg', 'admdvs', 'abbr', 'chgType', 'opterName', 'optTime'],
      excelCol: [
        { label: '组织机构名称', prop: 'orgName', width: '180px', align: 'left' },
        { label: '组织机构编码', prop: 'orgCodg', width: '180px', align: 'left' },
        { label: '医保区划', prop: 'admdvs', width: '180px', align: 'left' },
        { label: '简称', prop: 'abbr', width: '180px', align: 'left' },
  ...
  methods: {
    ...
    exportLocalData(type) {
      ...
      if (type === 'all') {
        ...
        excel({ params: { params: { pageNumber: 1, pageSize: this.pageInfo.total }, url: '/web/user/page/?' + '&pageNumber=1&pageSize=' + this.pageInfo.total, bizReqBody: {}, method: 'GET', cols: this.excelCol }}).then(response => {
          ...
          if (window.navigator.msSaveOrOpenBlob) {
            window.navigator.msSaveOrOpenBlob(blob, '组织机构变更信息.xlsx')
          } else {
            var downloadElement = document.createElement('a')
            var href = window.URL.createObjectURL(blob) // 创建下载的链接
            downloadElement.href = href
            downloadElement.download = '组织机构变更信息.xlsx' // 下载后文件名
      ...
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, '组织机构变更信息')
      })
    },
    // 格式化数据
