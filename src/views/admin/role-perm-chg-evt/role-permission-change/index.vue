...
<template>
  <!-- 角色权限变更 -->
  <normal-layer :search-number="itemsDatas.length" title-name="角色权限变更列表" title-need-handle>
    <template slot="search-header">
      <form-items ref="queryForm" :model="queryForm" :items-datas="itemsDatas" :form-datas="queryForm" :rules="rules">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      columnsLabel: ['角色名称', '资源名称', '资源类型', '变更类型', '经办人', '经办时间'],
      columnsValue: ['roleName', 'resuName', 'resuType', 'chgType', 'opterName', 'optTime'],
      excelCol: [
  ...
  methods: {
    ...
    getUserList() {
      ...
      this.loading = true
      ApiObj.rolePermChgPage(params).then(res => {
        if (res.code === 0) {
    ...
    exportLocalData(type) {
      ...
      if (type === 'all') {
        ...
        excel({ params: { params: { pageNumber: 1, pageSize: this.pageInfo.total }, url: '/web/user/page/?' + '&pageNumber=1&pageSize=' + this.pageInfo.total, bizReqBody: {}, method: 'GET', cols: this.excelCol }}).then(response => {
          ...
          if (window.navigator.msSaveOrOpenBlob) {
            window.navigator.msSaveOrOpenBlob(blob, '角色权限变更信息.xlsx')
          } else {
            var downloadElement = document.createElement('a')
            var href = window.URL.createObjectURL(blob) // 创建下载的链接
            downloadElement.href = href
            downloadElement.download = '角色权限变更信息.xlsx' // 下载后文件名
      ...
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, '角色权限变更信息')
      })
    },
    // 格式化数据
    formatJson(filterVal, jsonData) {
      for (const i in jsonData) {
        ...
        // if (jsonData[i].optTime) {
        //   parseTime(jsonData[i].optTime)
        // }
      }
