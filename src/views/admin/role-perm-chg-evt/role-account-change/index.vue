...
<template>
  <!-- 角色账号变更 -->
  <normal-layer :search-number="itemsDatas.length" title-name="角色账号变更列表" title-need-handle>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      columnsLabel: ['角色名称', '用户账号', '姓名', '角色类型', '变更类型', '经办人', '经办时间'],
      columnsValue: ['roleName', 'uact', 'userName', 'roleType', 'chgType', 'opterName', 'optTime'],
      excelCol: [
  ...
  methods: {
    ...
    exportLocalData(type) {
      ...
      if (type === 'all') {
        ...
        excel({ params: { params: { pageNumber: 1, pageSize: this.pageInfo.total }, url: '/web/user/page/?' + '&pageNumber=1&pageSize=' + this.pageInfo.total, bizReqBody: {}, method: 'GET', cols: this.excelCol }}).then(response => {
          ...
          if (window.navigator.msSaveOrOpenBlob) {
            window.navigator.msSaveOrOpenBlob(blob, '角色账号变更信息.xlsx')
          } else {
            var downloadElement = document.createElement('a')
            var href = window.URL.createObjectURL(blob) // 创建下载的链接
            downloadElement.href = href
            downloadElement.download = '角色账号变更信息.xlsx' // 下载后文件名
      ...
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, '角色账号变更信息')
      })
    },
    // 格式化数据
