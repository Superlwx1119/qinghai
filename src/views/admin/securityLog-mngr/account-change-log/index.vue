...
<template>
  <!-- 账号变更日志 -->
  <normal-layer :search-number="itemsDatas.length" title-name="账号变更日志列表" title-need-handle>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      columnsLabel: ['用户账号', '变更类型', '经办人', '经办时间'],
      columnsValue: ['uact', 'chgType', 'opterName', 'optTime'],
      excelCol: [
  ...
  methods: {
    ...
    getUserList() {
      ...
      this.loading = true
      ApiObj.uactChgPage(params).then(res => {
        if (res.code === 0) {
    ...
    exportLocalData(type) {
      ...
      if (type === 'all') {
        ...
        excel({ params: { params: { pageNumber: 1, pageSize: this.pageInfo.total }, url: '/web/user/page/?' + '&pageNumber=1&pageSize=' + this.pageInfo.total, bizReqBody: {}, method: 'GET', cols: this.excelCol }}).then(response => {
          ...
          if (window.navigator.msSaveOrOpenBlob) {
            window.navigator.msSaveOrOpenBlob(blob, '账号变更日志信息.xlsx')
          } else {
            var downloadElement = document.createElement('a')
            var href = window.URL.createObjectURL(blob) // 创建下载的链接
            downloadElement.href = href
            downloadElement.download = '账号变更日志信息.xlsx' // 下载后文件名
      ...
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, '账号变更日志信息')
      })
    },
    // 格式化数据
