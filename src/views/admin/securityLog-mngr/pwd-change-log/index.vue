...
<template>
  <!-- 密码变更日志 -->
  <normal-layer :search-number="itemsDatas.length" title-name="密码变更日志列表" title-need-handle>
    ...
    <my-table-view v-loading="loading" :columns="columns" :data="tableData" :have-expand="false" :max-cloumns="100" :is-configheader="false" :multiple-selection.sync="multipleSelection">
      <template slot="pwdChgType" slot-scope="scope">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      columnsLabel: ['用户账号', '密码变更类型', '经办人', '经办时间'],
      columnsValue: ['uact', 'pwdChgType', 'opterName', 'optTime'],
      excelCol: [
        { label: '用户账号', prop: 'uact', width: '180px', align: 'left' },
        { label: '密码变更类型', prop: 'pwdChgType', width: '180px', align: 'left' },
  ...
  methods: {
    ...
    formatJson(filterVal, jsonData) {
      for (const i in jsonData) {
        if (jsonData[i].pwdChgType === '1') {
          jsonData[i].pwdChgType = '修改'
        } else if (jsonData[i].pwdChgType === '2') {
          jsonData[i].pwdChgType = '重置'
        }
